...
gulp.task("build:dev", function() {
    ...
    return gulp.src(["src/untar.js"])
        ...
        .pipe(gulp.src(["src/ProgressivePromise.js", "src/untar-worker.js"])) // replace addSrc with gulp.src, merge streams if needed
...
gulp.task("build:dist", function() {
    // Process untar-worker.js first
    var workerStream = gulp.src("src/untar-worker.js")
        .pipe(sourcemaps.init())
        .pipe(jshint({ esversion: 6 }))
        .pipe(jshint.reporter("default"))
        .pipe(jshint.reporter("fail"))
    ...
    // Additional files stream
    var additionalFiles = gulp.src(["src/ProgressivePromise.js", "src/untar.js"])
        .pipe(jshint({ esversion: 6 }))
        .pipe(jshint.reporter("default"))
        .pipe(jshint.reporter("fail"));

    // Merge all streams, concat, wrap and uglify
    return merge(workerStream, additionalFiles)
